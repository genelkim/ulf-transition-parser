...
def get_amr_line(input_f):
    ...
    for line in input_f:
        line = line.strip()
        if line[:2] == "((":
            # extra paren wrapping format -- strip it off
...
def smart_init_mapping(candidate_mapping, instance1, instance2):
    ...
    for i, candidates in enumerate(candidate_mapping):
        if len(candidates) == 0 or len(instance1) < i + 1:
            # no possible mapping
            result.append(-1)
            continue
...
def print_alignment(mapping, instance1, instance2):
    ...
    for i, m in enumerate(mapping):
        if m == -1:
            if len(instance1) > i + 1:
                result.append(instance1[i][1] + "(" + instance1[i][2] + ")" + "-Null")
        else:
            if len(instance1) > i + 1 and len(instance2) > m + 1:
...
def main(arguments):
    ...
    while True:
        ...
        if cur_amr2 == "":
            ...
            print("Ignoring remaining AMRs", file=ERROR_LOG)
            break
        amr1 = el_amr.AMR.parse_AMR_line(cur_amr1)
        amr2 = el_amr.AMR.parse_AMR_line(cur_amr2)
        prefix1 = "a"
        prefix2 = "b"
...
if __name__ == "__main__":
    ...
    if sys.version_info[0] != 2 or sys.version_info[1] < 5:
        print("This script only supports python 2.5 or later. \
                It does not support python 3.x.", file=ERROR_LOG)
        exit(1)
    # use optparse if python version is 2.5 or 2.6
    if sys.version_info[1] < 7:
        import optparse
        if len(sys.argv) == 1:
            print("No argument given. Please run smatch.py -h \
                    to see the argument description.", file=ERROR_LOG)
            exit(1)
